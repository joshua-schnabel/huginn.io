/// Integration tests for the web UI — now using `hugin_web::server::run_server`.
...
async fn ui_index_html_contains_probe_name() {
    ...
    assert!(html.contains("hugin.dev"), "title not found in HTML");
    drop(hub);
}
...
async fn start_server(port: u16) -> Arc<EventHub> {
    ...
    tokio::spawn(async move {
        hugin_web::server::run_server(port, hub_clone).await.ok();
    });
    hub
}
