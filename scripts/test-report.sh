...
#
# Turn a captured `cargo llvm-cov --all` run (which executes the full test
...
# would miss. That is not hypothetical: it is what happened on v0.2.0, the first
# time release.yml ever ran here. `Running` never matched, so no suite was ever
...
# particular way is a parser that breaks on the next caller. muninn.io fixed
# this in its own release path first; this is the same fix.
