...
#
# before a single test runs. The version bump and the lock are not two changes,
# and treating them as two is how a release PR arrives red.
#
# Deliberately not `cargo update --workspace`, which would do the same job: the
# two callers are `release.yml`'s prepare-dev and `release-dispatch.yml`, both of
# which hold a write token, and `cargo` executes `build.rs` and proc-macros from
# the dependency tree. Resolution alone runs no build script, so the rule would
# arguably permit it — but this needs no toolchain, no network and no registry
# index, so there is nothing to weigh.
#
...
# substitution would also rewrite dependency version pins — it happens to be
# harmless today only because every pin in [workspace.dependencies] is inline.
